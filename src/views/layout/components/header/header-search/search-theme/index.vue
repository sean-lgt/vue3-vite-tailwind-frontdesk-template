  ...
  <div class="">
    ...
    <div class="flex h-[140px]" v-if="themeData.list.length">
      <div
        ...
        :style="{
          backgroundColor: createRandomRGB()
        }"
      >
        <img
...
<script setup>
import { ref } from 'vue'
import { getPexlesRecommendThemeApi } from '@/api/pexels.js'
...
const getSearchTheme = async () => {
  const resData = await getPexlesRecommendThemeApi()

  themeData.value = {
    big: resData.themes[0],
    list: resData.themes.splice(1, resData.themes.length)
  }
}
getSearchTheme()
</script>
